...
impl ContextCache {
    ...
    /// Get or load content
    pub async fn get_or_load<F, E>(&self, key: &str, path: &PathBuf, loader: F) -> Result<String, E>
    where
    ...
    {
        {
            ...
            if let Some(entry) = cache.get(key) {
                if !entry.is_expired(self.ttl) && !entry.is_stale(path) {
...
mod tests {
    ...
    async fn test_cache_hit_returns_cached() {
        ...
        // Second load without file change - should return cached
        let content = cache
            .get_or_load("key1", &file, async {
                Ok::<_, ()>(std::fs::read_to_string(&file).unwrap())
            })
    ...
    #[tokio::test]
    async fn test_cache_detects_file_modification() {
        let temp = tempdir().expect("Failed to create temp dir");
        ...
        // Load from file1
        let content = cache
            .get_or_load("key1", &file1, async {
                Ok::<_, ()>(std::fs::read_to_string(&file1).unwrap())
            })
            .await
            .unwrap();
        assert_eq!(content, "original");

        // Same key, different path (file2) - should load new content
        let content = cache
            .get_or_load("key1", &file2, async {
                Ok::<_, ()>(std::fs::read_to_string(&file2).unwrap())
            })
            .await
            .unwrap();
    ...
    async fn test_cache_lru_eviction() {
        ...
        for i in 0..3 {
            ...
            cache
                .get_or_load(&format!("key{}", i), &file.clone(), async move {
                    Ok::<_, ()>(std::fs::read_to_string(&file).unwrap())
                })
                .await
                .unwrap();
    ...
    async fn test_cache_invalidate_prefix() {
        ...
        for i in 0..3 {
            ...
            cache
                .get_or_load(&format!("group_{}", i), &file.clone(), async move {
                    Ok::<_, ()>(std::fs::read_to_string(&file).unwrap())
                })
                .await
                .unwrap();

...
impl MemoryBridge {
    ...
    pub async fn remember_to_file(
        ...
        // Add new lesson
        let lesson = format!("- **{}**: {}", chrono::Utc::now().format("%Y-%m-%d"), content);
...
use crate::memory::{TieredMemory, MigrationPolicy, Priority, TieredMemoryEntry};
...
impl UnifiedMemory {
    pub fn new(db_path: impl AsRef<std::path::Path>, file_root: impl AsRef<std::path::Path>) -> NuClawResult<Self> {
    ...
    pub async fn remember(&self, group: &str, key: &str, content: &str, priority: Priority) -> NuClawResult<()> {
    ...
    pub async fn add_preference(&self, group: &str, preference: &str) -> NuClawResult<()> {
        self.bridge.add_preference(group, preference).await
            .map_err(|e| crate::NuClawError::FileSystem { message: e.to_string() })?;
...
mod tests {
    ...
    async fn test_remember_to_file() {
        ...
        bridge.remember_to_file("test_group", "test_key", "Test lesson").await.unwrap();
        
        let path = temp.path().join("test_group").join("context").join("MEMORY.md");
        assert!(path.exists());
    ...
    async fn test_unified_memory_remember() {
        ...
        unified.remember("test_group", "test_key", "Test content", Priority::Normal).await.unwrap();
        
        let result = unified.recall("test_key").await.unwrap();
        assert!(result.is_some());
        assert_eq!(result.unwrap().content, "Test content");
        
        let path = temp.path().join("files").join("test_group").join("context").join("MEMORY.md");
    ...
    async fn test_unified_memory_load_from_file() {
        ...
        bridge.remember_to_file("test_group", "key1", "lesson content").await.unwrap();
    ...
    async fn test_unified_memory_add_preference() {
        ...
        unified.add_preference("test_group", "new_pref").await.unwrap();
        
        let tiered_result = unified.recall("preference:new_pref").await.unwrap();
        assert!(tiered_result.is_some(), "Preference should be stored in tiered memory");
    }
    ...
    async fn test_unified_memory_search() {
        ...
        unified.remember("test_group", "key1", "hello world", Priority::Normal).await.unwrap();
...
"#).unwrap();
    ...
    async fn test_add_preference() {
        ...
        bridge.add_preference("test_group", "new_preference").await.unwrap();
    ...
    async fn test_add_duplicate_preference() {
        ...
        bridge.add_preference("test_group", "dup_preference").await.unwrap();
        bridge.add_preference("test_group", "dup_preference").await.unwrap();
        
        let memory = bridge.load_from_file("test_group").await.unwrap();
        
        // Should only have one
        assert_eq!(memory.preferences.iter().filter(|p| *p == "dup_preference").count(), 1);
    }
}

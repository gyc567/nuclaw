...
impl ContextLoader {
    ...
    pub async fn load_identity(&self, context_dir: &Path) -> Result<Identity, ContextError> {
        ...
        Self::parse_yaml_frontmatter::<Identity>(&content)
            .map(|mut i| {
                // Also parse markdown body for additional traits
    ...
    pub async fn load_user(&self, context_dir: &Path) -> Result<User, ContextError> {
        ...
        Self::parse_yaml_frontmatter::<User>(&content)
            .or_else(|_| Ok(User::default_user()))
    }
    ...
    pub async fn load_memory(&self, context_dir: &Path) -> Result<Memory, ContextError> {
        ...
        Self::parse_yaml_frontmatter::<Memory>(&content)
    ...
    fn parse_yaml_frontmatter<T: serde::de::DeserializeOwned>(content: &str) -> Result<T, ContextError> {
...
"#;
        ...
        assert!(rules.startup_sequence.contains(&"load_identity".to_string()));
        assert!(rules.safety_boundaries.contains(&"Never expose private data".to_string()));
    }
...
"#;
        ...
        assert!(memory.lessons_learned.contains(&"Don't recommend steak restaurants".to_string()));
    }
    ...
    async fn test_load_context_full() {
        ...
        // Create all files
        std::fs::write(context_dir.join("SOUL.md"), "- name: Bot\n- role: Test").unwrap();
        std::fs::write(context_dir.join("USER.md"), "- name: User").unwrap();
        std::fs::write(context_dir.join("AGENTS.md"), "- version: 1.0").unwrap();
        std::fs::write(context_dir.join("MEMORY.md"), "- version: 1").unwrap();
    ...
    async fn test_security_path_traversal() {
        ...
        // Try to load from outside base path
